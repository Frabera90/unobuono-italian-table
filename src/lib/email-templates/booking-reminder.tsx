...
const BookingReminder = ({ customerName, restaurantName, date, time, partySize, manageUrl }: Props) => (
  <Html lang="it" dir="ltr">
    <Head />
    <Preview>{`Domani alle ${time ?? ''}${restaurantName ? ` — ${restaurantName}` : ''}`}</Preview>
    <Body style={main}>
      <Container style={container}>
        <Heading style={h1}>Ti aspettiamo domani 🍽</Heading>
        <Text style={text}>
          {customerName ? `Ciao ${customerName},` : 'Ciao,'} ti ricordiamo che hai una prenotazione{restaurantName ? ` da ${restaurantName}` : ''} domani.
        </Text>
        <Section style={card}>
        ...
        {manageUrl && (
          <>
            <Text style={text}>Vuoi pre-ordinare o hai bisogno di modificare la prenotazione?</Text>
            <Button href={manageUrl} style={button}>Gestisci prenotazione</Button>
          </>
        )}
        <Hr style={hr} />
        <Text style={footer}>A domani!{restaurantName ? ` — ${restaurantName}` : ''}</Text>
      </Container>
    </Body>
  </Html>
)
...
export const template = {
  ...
  subject: (d: Record<string, any>) => `Ti aspettiamo domani${d?.time ? ` alle ${d.time}` : ''}`,
  displayName: 'Reminder prenotazione',
  previewData: {
    customerName: 'Marta',
