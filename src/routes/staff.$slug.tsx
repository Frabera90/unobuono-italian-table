...
export const Route = createFileRoute("/staff/$slug")({
  ...
  component: StaffSlugPage,
});

function StaffSlugPage() {
  ...
  useEffect(() => {
    // Look up restaurant name by slug for display
    void supabase
      .from("restaurants")
      .select("id, name")
      .eq("slug", slug)
  ...
  useEffect(() => {
    ...
    const existingRestId = localStorage.getItem("staff.restaurant_id");
    ...
    if (existingRestId) nav({ to: "/waiter" });
  }, [nav, pinParam]);
  ...
  async function submit(e: React.FormEvent) {
    ...
    setBusy(true);
    try {
      // Verify PIN and get restaurant_id
      const { data: restaurantId, error } = await supabase.rpc("restaurant_id_by_staff_pin", {
        _pin: pin.trim(),
      });
      ...
      // Verify the restaurant_id matches the slug in the URL
      ...
      if (!restaurant) {
        toast.error("PIN non valido per questo ristorante.");
        return;
      }

      localStorage.setItem("staff.restaurant_id", restaurantId as string);
    ...
    <div className="grid min-h-screen place-items-center bg-ink px-5 py-10 text-paper">
      <div className="w-full max-w-sm">
        <div className="mb-6 flex flex-col items-center text-center">
          ...
          {restaurantName ? (
            <p className="mt-2 text-sm font-medium text-paper/80">{restaurantName}</p>
          ) : (
            <p className="mt-2 text-sm text-paper/60">Accesso staff</p>
          )}
        </div>
        ...
        <form onSubmit={submit} className="space-y-4 rounded-2xl border border-white/10 bg-white/5 p-6">
          ...
          <label className="block">
            ...
            />
            <span className="mt-1 block text-[11px] text-paper/50">
              {pinParam ? "PIN ricevuto dal titolare" : "Chiedi il PIN al titolare"}
            </span>
          </label>
